...
from .config import D_MODEL, ENCODER_LR, FF_EXPANSION, KPTS_FEAT_DIM, MBART_DIM, N_HEADS, SIGLIP_DIM
...
class SqueezeformerFusionEncoder(nn.Module):
    ...
    def forward(self, kpts, kpts_mask, siglip):
...
class SignTranslationModel_15(nn.Module):
    def __init__(self, encoder, text_model: str = "facebook/mbart-large-cc25"):
        super().__init__()
    ...
    def generate(self, batch, tokenizer, max_new_tokens: int = 60):
        self.eval()
        with torch.no_grad():
            enc_out, _ = self.encoder(batch["kpts"], batch["kpts_mask"], batch["siglip"])
            enc_out = self.adapter(enc_out)
            ids = self.mbart.generate(
                inputs_embeds=enc_out,
                attention_mask=batch["kpts_mask"],
